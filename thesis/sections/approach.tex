...
%In addition to the aforementioned requirements, the approach has to provide mitigation strategies against various types of attacks in order to secure an SAS.
...
A plethora of different threats, adversaries, and cyberattack classifications applicable to SCADA, SAS, ICS, or smart grid systems are discussed in the literature \cite{Nicholson2012,Aloul2012,Rizzetti2015,Cai2016,Hussain2021}.
...
In order to achieve this objective, the adversary may either replay messages that have been captured on the network or modify them.
...
An adversary may achieve disruption of the device's availability via the deployment of malware or (distributed) denial of service (DoS) attacks.
Examples of such DoS attacks in a SAS include the intentional delay of time-critical messages, also known as time-delay attacks, and flooding attacks.
...
\todo{TODO: Security Policies -> Link to Requirements \& Attacks}
...
\subsubsection{Policy Specification Language}
\todo{Write + Remove duplicate ideas from protocols}
\paragraph{Policy Structure}
\paragraph{Attribute Types}
\paragraph{Grammar}

\subsection{Access Control Decision}
\todo{TODO: Write + Remove duplicate ideas from protocols}

\subsubsection{Evaluation Strategies}
\todo{TODO: Ad-hoc vs. precomputation}
