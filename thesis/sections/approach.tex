...
The partial private key generation algorithm $ppk_A \leftarrow G_{PPK}(\rho, s, ATT_A, pk_A)$ takes the public system parameters $\rho$, the private master key of the CAPP $s$, the defining attributes of entity $A$ $ATT_A$, and the $A$'s public verification key $pk_A$ as input.
The algorithm outputs the partial private key $ppk_A$ of entity $A$.
The partial private key generation is executed by the CAPP on request of an entity.
...
The receiver of a message executes the partial entity verification algorithm $V_{ENT}$ and sends the partially verified signature $\sigma_{ENT}$ to the CAPP for server-aided verification.
...
The algorithm is executed by the CAPP on request of a message receiving entity.
After the execution, the CAPP provides the partially verified signature $\sigma_{SAV}$ to the requestor.
