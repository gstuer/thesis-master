...
\todo{Overhaul benefits and challenges based on Guide to Attribute Based Access Control (ABAC) Definition and Considerations Page 8}
\todo{Describe/Present architecture of XACML based on RFC 2904 including PEP, PDP, and PIP}
...
NIST recommends considering logical access control minimizing errors and costs of maintaining access privileges.
...
Subsequently, Alice uses $s$ to encrypt a plaintext message $m$ and transmits the ciphertext message $c_m$ to Bob via the unsecure communication channel.
Upon receipt, Bob retrieves $m$ by using $s$ to decrypt $c_m$.
...
For the same level of security, the symmetric cryptographic keys are shorter.
The latter advantage reduces the required memory for key storage and bandwidth for key transmission.

\subsection{Asymmetric Cryptography (Public-Key Cryptography)}
\todo{TODO Asymmetric concept and examples and dis/advantages}
\todo{TODO PKI}
