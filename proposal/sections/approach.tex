...
\begin{description}
    ...
    \begin{figure}
        \centering
        \includegraphics[width=1.0\linewidth]{figures/SABAAC_protocols_authorization_static.drawio.pdf}
        \caption{Components and exchanged messages of the static authorization and policy persisting process initiated by a policy modification request of a human operator at a HMI.}
...
\begin{description}
    ...
    Moreover, the requesting PEP may send the session initialization request along with a domain-specific request by piggybacking a payload exchange request.
    The usage of piggybacking decreases the required time until a domain-specific response arrives at the requesting PEP by one RTT.
    ...
    If the request is invalid, the PEP sends a negative initialization acknowledgement to the requestor.
    ...
    The goal of the payload exchange phase is to exchange signed and authorized domain-specific requests between a sender and a receiver.
    For this purpose, the sender's PEP appends a valid session-specific sequence number to a domain-specific request.
    This extended request is digitally signed and sent to the receiver's PEP.
