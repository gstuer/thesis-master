...
The second core concept of the approach is the Server-Aided Attribute-Based Access Control (SABAC).
This concept provides mechanisms to enable Attribute-Based Access Control (ABAC) for time-critical SAS communication.
...
This concept is further discussed in \autoref{sec:approach:sabac}.
...
The SABAC concept of CASC-SAS is part of this architectural layer.
SABAC provides authorization and access control to satisfy the security requirements confidentiality, PoLP, and SoD.
%Provides authorization and access control in order to satisfy confidentiality, PoLP, and SoD.
...
%Provides authentication via digital signatures in order to satisfy integrity, authenticity, non-repudiation, and privacy preservation.
...
The yet unsigned and non-authorized messages are then authorized by SABAC and forwarded to CASA at the cryptography layer for signing.
...
The second core concept of the CASC-SAS approach is the \textbf{S}erver-Aided \textbf{A}ttribute-\textbf{B}ased \textbf{A}ccess \textbf{C}ontrol (SABAC).
The SABAC approach enables the employment of attribute-based authorization and access control for time-critical SAS communication.
Therefore, SABAC prevents unauthorized access and extraction of information.
...
SABAC represents a cybersecurity concept that is located on the cybersecurity layer of the CASC-SAS architecture.
Since the approach is located on the cybersecurity layer, it relies on secure authentication services provided by CASA.
As a consequence, SABAC assumes that efficient and secure signing and verification algorithms are available.
In other words, CASA provides secure cryptographic algorithms and schemes that enable SABAC to realize secure authorization and access control.

The SABAC approach is divided into two central tasks.
The first task is referred to as delegated attribute-based authorization.
The delegated authorization process is further discussed in \autoref{sec:approach:sabac:authorization}.
The second central task is referred to as delegated ABAC.
The delegated ABAC process is further discussed in \autoref{sec:approach:sabac:accesscontrol}.
\todo{Security Requirements: Also integrity but not really confidentiality provided by SABAC?}

\subsection{Delegated Attribute-Based Authorization}
\label{sec:approach:sabac:authorization}
\todo{Policies (Rule/Pol Types:RT/Static, DSL, appropriate message types for pol. types)}
\todo{Hierarchic PDP/PAPs}
\todo{Authorization Protocol: Static vs Dynamic Auth.}

\subsection{Delegated Attribute-Based Access Control}
\label{sec:approach:sabac:accesscontrol}
